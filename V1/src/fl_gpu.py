...
def create_topology_mnist_conv_28_28_1():
  seed(1)
  tf.compat.v1.random.set_random_seed(1) #tf.random.set_random_seed(1)
  model = Sequential()
...
max_epochs, max_local_steps, max_global_aggregations):
      ...
      self.comm_quant = 0
      for i in range(len(quantization_precission)):
          self.comm_quant += (quantization_precission[i] / 32.0) * percentage_params_per_layer[i]
      self.comm_n_devices = selected_workers_per_aggregation / n_slaves
      self.comm_thres = 0
      for i in range(len(threshold_percentage)):
          self.comm_thres += ((100.0 - threshold_percentage[i]) / 100.0) * percentage_params_per_layer[i]

      self.comm_steps = ((self.total_m_batches_local + 1.0) - local_steps) / self.total_m_batches_local
      self.communication_fitness = self.comm_quant * self.comm_n_devices * self.comm_thres * self.comm_steps

      
      print("self.total_m_batches_local = " + str(self.total_m_batches_local))


      print("self.comm_quant = " + str(self.comm_quant))
      print("self.comm_n_devices = " + str(self.comm_n_devices))
      print("self.comm_thres = " + str(self.comm_thres))
      print("self.comm_steps = " + str(self.comm_steps))
      print("self.communication_fitness = " + str(self.communication_fitness))
          ...
          for i in range(len(self.models[selected_devices[0]].trainable_variables)):
              ...
              for device in selected_devices:
                  if self.threshold_percentage[i] > 0:
                    threshold = tf.contrib.distributions.percentile(tf.math.abs(self.models[device].trainable_variables[i]), q=self.threshold_percentage[i]) #tfp.stats.percentile(tf.math.abs(self.models[device].trainable_variables[i]), q=self.threshold_percentage[i])
                  else:
                    threshold = 0
...
class JSDEvaluate():
  ...
  def evaluate(self, solution):
    ...
    local_steps = 100
    selected_workers_per_aggregation = n_slaves
